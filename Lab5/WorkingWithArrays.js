...
export default function WorkingWithArrays(app) {
  ...
  const updateTodoTitle = (req, res) => {
    ...
    if (!todo) {
      res
        .status(404)
        .json({ message: `Unable to update title for Todo ID ${id}` });
      return;
    }
    todo.title = title;
    res.json(todos);
  ...
  const updateTodoCompleted = (req, res) => {
    ...
    if (!todo) {
      res
        .status(404)
        .json({ message: `Unable to update completion for Todo ID ${id}` });
      return;
    }
  ...
  const updateTodoDescription = (req, res) => {
    ...
    if (!todo) {
      res
        .status(404)
        .json({ message: `Unable to update description for Todo ID ${id}` });
      return;
    }
